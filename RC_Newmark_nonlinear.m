function [C,str,K,Gs,D,amp,t,acc,vel,u,pt]=RC_Newmark_nonlinear(G0,Gur,g07,Jm,diam,L,p,T0,f,dt,tt,tf,Dmin)
...
for ii=2:time
    ...
    tol=1e-10;       %Default tolerance set to 1e-10
    iter=1;         %First iteration
    while abs(err) >= tol
        str(ii)=RC_strain(base,u(ii),diam,L);
        def_base=RC_strain(0,base,diam,L);
        [Gs(ii),~,D(ii)]=non_linearGD(str(ii),def_base,G0,g07,Gur);
        D(ii)=D(ii)+Dmin;
