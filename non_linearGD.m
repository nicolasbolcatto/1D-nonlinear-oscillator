function [Gs,Gt,D]=non_linearGD(defG,defD,G0,g07,Gur)
...
if defD>0
    if defD >= g_cutoff
        vv=4*G0*g07/0.385;
        ww=2*g_cutoff;
        xx=g_cutoff/(1+(g07/(0.385*g_cutoff)));
        yy=2*g07/0.385;zz=log(1+0.385*g_cutoff/g07);
        ED=vv*(ww-xx-yy*zz);
        ES=G0*g_cutoff*g_cutoff/(2+2*0.385*g_cutoff/g07);
        D=(ED/(4*pi*ES));
    else
        vv=4*G0*g07/0.385;
        ww=2*defD;
        xx=defD/(1+(g07/(0.385*defD)));
        yy=2*g07/0.385;zz=log(1+0.385*defD/g07);
        ED=vv*(ww-xx-yy*zz);
        ES=G0*defD*defD/(2+2*0.385*defD/g07);
        D=(ED/(4*pi*ES));  
    end
else
    D=0;
